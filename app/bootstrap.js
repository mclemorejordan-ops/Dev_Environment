/********************
 * bootstrap.js — Phase 3.6
 * App bootstrap / wiring moved out of app.js
 ********************/
...
// Bind modal controls once on boot
...
export function initBootstrap(){
  ...
  const Library = initLibrary({ getState: () => state, Storage, uid });
  ...
  // Routines engine
  const Routines = initRoutinesEngine({
    getState: () => state,
    Storage,
    uid,
    ExerciseLibrary,
  ...
  /********************
   * UIState + small helpers
   ********************/
  const UIState = {
    protein: { dateISO: Dates.todayISO() }
  };
