/********************
 * views.js — Phase 3.7
 * Extracted Views object from app.js
 *
 * Key design:
 * - Views code can keep using `state.*` by using a live proxy for property access.
 * - Any time Views calls Storage.save(state) / Storage.flush(state), we inject a wrapped Storage
 *   whose save/flush ignore the passed arg and always save the real getState().
 * - Same for BackupVault.forceSnapshot(state, ...).
 * - Any state "reassign" (state = ...) is replaced with setState(...).
 ********************/
...
export function initViews(ctx){
  const {
    ...
    // injected (wrapped) persistence helpers
    Storage,
    BackupVault,
    ...
    // workouts / logs / progress / attendance + helpers
    LogEngine,
    ...
    // attendance-ui extracted helpers + engine
    isTrained,
    toggleTrained,
    ymFromISO,
    monthTitle,
    ...
    // protein-ui extracted helpers
    buildProteinTodayModal,
    deleteMeal,
    totalProtein,

    // logs.protein helpers (some views reference these directly)
    findProteinEntry,
    ensureProteinEntry,
    ...
    // app-level UI state (persisted in window)
    UIState,
  ...
  if(typeof getState !== "function") throw new Error("initViews requires getState()");
  if(typeof setState !== "function") throw new Error("initViews requires setState()");
  if(!Storage) throw new Error("initViews requires Storage");
  if(!BackupVault) throw new Error("initViews requires BackupVault");
  if(typeof el !== "function") throw new Error("initViews requires el()");
  if(typeof navigate !== "function") throw new Error("initViews requires navigate()");

  // Live proxy so Views can keep doing `state.profile...` without stale references.
  // IMPORTANT: do NOT pass this proxy to JSON/stringify; use getState() for persistence.
  ...
  /********************
   * VIEWS (extracted verbatim from app.js, with only 3 changes):
   * 1) state = Storage.load();        -> setState(Storage.load());
   * 2) state = migrateState(snap...)  -> setState(migrateState(snap...));
   * 3) state = Storage.load();        -> setState(Storage.load());
   ********************/
