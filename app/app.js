...
root.appendChild(el("div", { class:"card" }, [
  ...
  const emptyStateNode = (() => {
    ...
    return null;
       ...
       // ─────────────────────────────
  // My Profile — Highlights (UI-only)
  // ─────────────────────────────
  const highlightsNode = (() => {
    if(!(user && viewBody === "profile")) return null;
    ...
    const fmtInt = (n) => {
      ...
      try{ return String(x).replace(/\B(?=(\d{3})+(?!\d))/g, ","); }
      catch(_){ return String(x); }
    };

    const fmtTime = (sec) => {
      const s = Math.max(0, Math.round(safeNum(sec)));
    ...
    const fmtMi = (d) => {
      const n = safeNum(d);
      if(!n) return "0.00 mi";
      return `${n.toFixed(n >= 10 ? 1 : 2)} mi`;
    };

    const fmtPace = (paceSec) => {
      const s = Math.max(1, Math.round(safeNum(paceSec)));
      const m = Math.floor(s / 60);
      const r = s % 60;
      // Neutral label: fastest pace, formatted like running apps
      return `${m}:${String(r).padStart(2,"0")}/mi`;
    };

    // Metrics derived from *your* events already in feedList.
    let bestLift = { w: 0, name: "", when: "" };
    let cardioBest = { pace: 0, dist: 0, time: 0, when: "" };
    let prsShared = 0;

    // Consistency = unique active days in last 14 days (based on posts)
    ...
    (feedList || []).forEach(ev => {
      const p = ev?.payload || {};
      const createdAt = ev?.createdAt || p?.createdAt;
      ...
      // PRs shared (workout + exercise events)
      prsShared += safeNum(p?.highlights?.prCount);
      prsShared += safeNum(p?.prCount);

      // Best lift shared (prefer exercise_logged bestWeight-ish fields)
      const bw = safeNum(
        p?.summary?.bestWeight ||
        p?.summary?.topWeight ||
        p?.summary?.maxWeight ||
        p?.bestWeight ||
        p?.topWeight ||
      ...
      if(bw > (bestLift.w || 0)){
        bestLift = {
          w: bw,
          name: exName || "Best lift",
          when: Number.isFinite(ts) ? new Date(ts).toLocaleDateString() : ""
        };
      }

      // Cardio best = fastest pace (lowest paceSecPerUnit)
      const pace = safeNum(
        p?.summary?.paceSecPerUnit ||
        p?.summary?.paceSecPerMi ||
        p?.paceSecPerUnit ||
        p?.paceSecPerMi
      );
      ...
      const dist = safeNum(
        p?.summary?.distance ||
        p?.summary?.distanceMi ||
        p?.distance ||
        p?.distanceMi
      );

      const timeSec = safeNum(
        p?.summary?.timeSec ||
        p?.summary?.durationSec ||
        p?.timeSec ||
        p?.durationSec
      );

      if(pace > 0){
        if(!cardioBest.pace || pace < cardioBest.pace){
          cardioBest = {
            pace,
            dist,
            time: timeSec,
    ...
    // Card text (safe fallbacks)
    const bestLiftMain = bestLift.w > 0 ? `${bestLift.name} • ${fmtInt(bestLift.w)} lb` : "No lift shared yet";
    const bestLiftSub  = bestLift.w > 0 ? (bestLift.when ? `From ${bestLift.when}` : "From a recent post") : "Log & share a lift to populate";

    const cardioMain = cardioBest.pace > 0
      ? `${fmtPace(cardioBest.pace)}${cardioBest.dist ? ` • ${fmtMi(cardioBest.dist)}` : ""}${cardioBest.time ? ` • ${fmtTime(cardioBest.time)}` : ""}`
      : "No cardio shared yet";
    const cardioSub  = cardioBest.pace > 0 ? "Fastest pace shared" : "Log & share cardio to populate";

    const consistencyMain = `${consistencyPct}%`;
    const consistencySub  = `${activeDays}/14 days active`;

    const prsMain = fmtInt(prsShared);
    const prsSub  = "PRs shared (all-time)";

    const kpi = (label, main, sub) => el("div", {
      style:[
      ...
      el("div", { class:"note", style:"margin:0; opacity:.62; font-weight:950; letter-spacing:.2px; font-size:11px;", text: label }),
      el("div", { style:"font-size:16px; font-weight:1000; margin-top:6px;", text: main }),
      el("div", { class:"note", style:"margin:0; opacity:.78; margin-top:4px; font-size:12px; line-height:1.25;", text: sub })
    ]);
    ...
    return el("div", {
      ...
      el("div", { style:"font-weight:1000; letter-spacing:.2px; font-size:13px; opacity:.92;", text:"Highlights" }),
      el("div", { style:"height:10px" }),
      el("div", { style:"display:grid; grid-template-columns:1fr 1fr; gap:10px;" }, [
        kpi("Best Lift Shared", bestLiftMain, bestLiftSub),
        kpi("Cardio Best", cardioMain, cardioSub),
        kpi("Consistency", consistencyMain, consistencySub),
        kpi("PRs Shared", prsMain, prsSub),
      ])
    ]);
  })();
