...
root.appendChild(el("div", { class:"card" }, [
  ...
  const emptyStateNode = (() => {
    ...
    const fmtInt = (n) => {
      try{ return String(Math.round(safeNum(n))).replace(/\B(?=(\d{3})+(?!\d))/g, ","); }
      catch(_){ return String(Math.round(safeNum(n))); }
    };
    ...
    const bestLiftMain = bestLift.w > 0
      ? `${bestLift.name} • ${fmtInt(bestLift.w)}`
      : "No lift shared yet";

    const bestLiftSub = bestLift.w > 0
      ? (bestLift.when ? `From ${bestLift.when}` : "From a recent post")
      : "Log & share a lift to populate";

    const cardioMain = cardioBest.pace > 0
      ? `${fmtPace(cardioBest.pace)}${cardioBest.dist ? ` • ${cardioBest.dist}` : ""}${cardioBest.time ? ` • ${fmtTime(cardioBest.time)}` : ""}`
      : "No cardio shared yet";

    const cardioSub = cardioBest.pace > 0
      ? (cardioBest.when ? `From ${cardioBest.when}` : "From a recent post")
      : "Log & share cardio to populate";
